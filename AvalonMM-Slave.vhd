...
architecture rtl of AvalonMM_Slave is
  ...
  -- Функции (переменные в функциях допустимы)
  ...
  -- Регистры (все счетчики как std_logic_vector)
...
begin
  ...
  -- FSM для работы с Avalon-MM мастером - только управление состоянием
  ...
  -- Логика данных для master FSM
  master_data_process: process(clk, reset_n)
    variable burst_len_int      : integer;
    variable active_bytes_int   : integer;
  ...
  begin
    if reset_n = '0' then
      ...
      burst_len_int := to_integer(unsigned(burstcount));
      if burst_len_int = 0 then 
        burst_len_int := 1; 
      end if;
      active_bytes_int  := count_ones(byteenable);
      byte_offset_int   := lowest_enabled_byte(byteenable);
      ...
      case master_state_r is
        when IDLE =>
          if write = '1' and internal_state_r = INT_IDLE then
            ...
            captured_byte_offset_r <= std_logic_vector(to_unsigned(byte_offset_int, 8));
            captured_active_bytes_r <= std_logic_vector(to_unsigned(active_bytes_int, 8));
  ...
  -- FSM для работы с внутренней логикой - только управление состоянием
  ...
  -- Логика данных для internal FSM
  internal_data_process: process(clk, reset_n)
  ...
  begin
    if reset_n = '0' then
      ...
      total_bytes_v := to_unsigned(to_integer(unsigned(captured_active_bytes_r)) * to_integer(unsigned(captured_burst_r)), 16);
      adj_addr := unsigned(captured_addr_r) + unsigned(captured_byte_offset_r);
          ...
          if resp_empty = '0' then
            ...
            if to_integer(unsigned(captured_active_bytes_r)) > 0 then
              internal_beats_remaining_r <= std_logic_vector(
                (unsigned(remaining_bytes_r) + unsigned(captured_active_bytes_r) - 1) / unsigned(captured_active_bytes_r)
              );
            else
              internal_beats_remaining_r <= (others => '0');
            end if;
